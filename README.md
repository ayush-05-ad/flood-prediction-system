...
**Guide:** Asst. Prof. Aparna Kamal | **HOD:** Manoj Kumar Shah
...
│   └── sensor_node.ino          # Main firmware (v3.0 - 3 sensors + JSON + alerts)
│
├── ml_training/                 # AI/ML Model (Google Colab)
│   ├── Flood_LSTM_Training.ipynb # Complete training notebook
│   ├── flood_model.h5           # Trained LSTM model (1.04 MB)
│   ├── flood_model.keras        # Keras format model
│   ├── data_scaler.pkl          # MinMaxScaler for normalization
│   └── prediction_results.png   # Actual vs Predicted graph
│
├── flutter_app/                 # Mobile App (Flutter)
│   └── main.dart                # Complete app with Day/Night mode
│
├── edge_ai/                     # Raspberry Pi Gateway (Python)
│   ├── main.py                  # Gateway main loop
│   ├── lstm_predictor.py        # TFLite inference module
│   ├── risk_scorer.py           # Village risk assessment
│   ├── alert_engine.py          # SMS + Push + Siren alerts
│   └── lora_gateway.py          # LoRa receiver
│
├── web_dashboard/               # React Dashboard (Coming Soon)
│
├── docs/                        # Documentation
│
└── README.md
...
Dense (64, ReLU) → Dropout (0.2) → Dense (32, ReLU)
    ↓
Output: [24h_prediction, 48h_prediction, 72h_prediction]
```
